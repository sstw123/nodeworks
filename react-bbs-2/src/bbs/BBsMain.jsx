...
import BBsList from "./BBsList.jsx"
...
class BBsMain extends Component {
    ...
    */
    ...
    componentDidMount() {
        ...
        // 이전에 callback 함수가 종료되고 timer 시간이 경과하면 다시 반복하여 실행
        this.timer = setInterval(() => this.fetchBBsList(), 5000)
    }

    // 화면에 그려지는 여러 컴포넌트들의 연결이 깨지면 실행되는 메소드
    // timer가 계속 작동되면 react 실행히 deadlock에 빠질 수 있기 때문에 반드시 WillUnmount() method에서 timer를 멈춰줘야 한다
    componentWillUnmount() {
        this.timer = null
    }

    //bbs 서버에서 리스트를 조회하는 method
    fetchBBsList = () => {
        this.setState({
            ...this.state,
            isFetch : true
    ...
    render() {
        /*
            ...
            필드영역에 선언된 this.state를 분해하여 그 중 bbsList를 별도로 추출해두면
